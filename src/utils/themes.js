
export const LightTheme = {
    bg: "#251451",         // Light gray background
    bgLight: "#251451",    // White background
    primary: "#4A90E2",   // A blockchain blue color
    text_secondary: "#fd98ff",  // Dark text for content
    text_primary: "#001d6d",  // Slightly lighter secondary text
    card: "#e0e0e0",         // Light gray card background
    card_light: "#251451",   // Slightly lighter card background
    button: "#2E7D32",       // Green button color for actions
    white: "#FFFFFF",        // Pure white
    black: "#000000",        // Pure black
  }
  

/**@author: Ahmet CAN EROL
 * Date: 28.10.23
 * Project: Proof of Evoulation
 */